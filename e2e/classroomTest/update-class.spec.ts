...
await page.goto("http://localhost:3000/class/1/edit");
    ...
    await expect(page).toHaveURL("http://localhost:3000/class/1/edit");
    await page.getByRole('tab', { name: 'Fill class detail' }).click();
    await page.getByPlaceholder('Enter class details').click();
    await page.getByPlaceholder('Enter class details').fill("This lecture covers fundamental cybersecurity principles, common threats, and practical strategies to protect digital assets. (edited)");
...
test("should be update publish class", async ({page}) => {
    ...
    await page.route("http://localhost:8080/api/class/1/set-publish-status", route => {
        route.fulfill({
    ...
    await page.getByLabel('Publish').nth(1).click();

    await expect(page.getByRole('alert')).toContainText('Classroom is published');
})
...
await page.goto("http://localhost:3000/class/1/edit");
    ...
    await page.getByRole('tab', { name: 'Fill class detail' }).click();
    await page.getByPlaceholder('Enter class details').click();
    await page.getByPlaceholder('Enter class details').fill("");
